...
const randomSeed = numberArg(args.seed, 411000, "--seed");
...
const result = await generateFixedHandBiddingMarginDataset({
  ...
  onProgress: (progress) => {
    if (progress.completedPairs === progress.totalPairs || progress.completedPairs % 25 === 0) {
      console.error(`pairs ${progress.completedPairs}/${progress.totalPairs}; rollouts ${progress.completedRollouts}/${progress.totalRollouts}`);
    }
  }
});
...
function usage(message) {
  ...
  console.error("usage: node scripts/generate-fixed-hand-bidding-margin-dataset.mjs --output /tmp/ds --pairs 1000 --repeats 50 [--reserved-hands /tmp/issue409-selected-hands.json]");
  process.exit(1);
}
