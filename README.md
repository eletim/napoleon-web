...
NN学習基盤の最初の単位として、指定seedで5人のルールベースAI対戦を最後まで実行し、判断ログを収集できます。同じseedでは配札と行動を再現できるよう、ゲーム用乱数と各Agent用乱数を分離しています。

記録ではAgentから見えていた観測と、学習・デバッグ用の実手札ラベルを分離しています。ブラウザの`AI対戦ログ`画面では単一対戦の概要、初期配札、集計、判断タイムラインを確認できます。表示される全手札は教師ラベル用の完全情報です。
