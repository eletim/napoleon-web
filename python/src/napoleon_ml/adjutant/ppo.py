"""Adjutant-only PPO training utilities for non-playing RL samples."""
...
class AdjutantPpoCompatibilityError(ValueError):
    """Raised when a adjutant PPO artifact is incompatible with the current schema."""


@dataclass(frozen=True)
class NonPlayingAdjutantRlManifest:
...
@dataclass(frozen=True)
class NonPlayingAdjutantRlSample:
    seed: int
    step: int
...
class AdjutantPpoBatch(NamedTuple):
    model_input: Tensor
...
@dataclass(frozen=True)
class AdjutantPpoTrainSettings:
    seed: int
    epochs: int
    batch_size: int
    learning_rate: float
...
class NonPlayingAdjutantRlDataset(Dataset[NonPlayingAdjutantRlSample]):
    def __init__(self, samples: list[NonPlayingAdjutantRlSample]) -> None:
        if len(samples) == 0:
            raise AdjutantPpoCompatibilityError("non-playing adjutant RL dataset has no samples.")
        self._samples = samples

    def __len__(self) -> int:
        return len(self._samples)

    def __getitem__(self, index: int) -> NonPlayingAdjutantRlSample:
        return self._samples[index]


def load_non_playing_adjutant_rl_manifest(
    dataset_directory: Path | str,
) -> NonPlayingAdjutantRlManifest:
    path = Path(dataset_directory) / "manifest.json"
    try:
        raw = json.loads(path.read_text(encoding="utf-8"))
    except OSError as error:
        raise AdjutantPpoCompatibilityError(f"manifest cannot be read: {error}") from error
    except json.JSONDecodeError as error:
        raise AdjutantPpoCompatibilityError(f"manifest is not valid JSON: {error}") from error

    if not isinstance(raw, dict):
        raise AdjutantPpoCompatibilityError("manifest must be a JSON object.")
    if raw.get("sampleType") != NON_PLAYING_RL_SAMPLE_TYPE:
        raise AdjutantPpoCompatibilityError(
            f"manifest sampleType must be {NON_PLAYING_RL_SAMPLE_TYPE!r}."
        )
    if raw.get("phaseScope") != "adjutant-only":
        raise AdjutantPpoCompatibilityError("manifest phaseScope must be 'adjutant-only'.")
    if raw.get("adjutantEncoderSchemaVersion") != ADJUTANT_ENCODER_SCHEMA_VERSION:
        raise AdjutantPpoCompatibilityError("manifest adjutantEncoderSchemaVersion mismatch.")
    if raw.get("adjutantModelInputSchemaVersion") != ADJUTANT_MODEL_INPUT_SCHEMA_VERSION:
        raise AdjutantPpoCompatibilityError("manifest adjutantModelInputSchemaVersion mismatch.")
    if raw.get("adjutantModelInputFeatureCount") != ADJUTANT_MODEL_INPUT_FEATURE_COUNT:
        raise AdjutantPpoCompatibilityError("manifest adjutantModelInputFeatureCount mismatch.")
    if raw.get("actionCount") != CARD_COUNT:
        raise AdjutantPpoCompatibilityError("manifest actionCount mismatch.")
    if raw.get("cardIdsSha256") != calculate_card_ids_sha256():
        raise AdjutantPpoCompatibilityError("manifest cardIdsSha256 mismatch.")
    ...
    if (
        ...
        raise AdjutantPpoCompatibilityError("manifest reward metadata mismatch.")
    ...
    return NonPlayingAdjutantRlManifest(
...
def iter_non_playing_adjutant_rl_samples(
    dataset_directory: Path | str,
) -> Iterator[NonPlayingAdjutantRlSample]:
    directory = Path(dataset_directory)
    manifest = load_non_playing_adjutant_rl_manifest(directory)
    shards = manifest.raw.get("shards")
    if not isinstance(shards, list):
        raise AdjutantPpoCompatibilityError("manifest.shards must be a list.")
    ...
    for shard_index, shard in enumerate(shards):
        ...
        if not isinstance(file_name, str) or "/" in file_name or "\\" in file_name:
            raise AdjutantPpoCompatibilityError("manifest shard file must be a bare file name.")
        shard_path = directory / file_name
        try:
            lines = shard_path.read_text(encoding="utf-8").splitlines()
        except OSError as error:
            raise AdjutantPpoCompatibilityError(f"shard cannot be read: {error}") from error
...
def create_non_playing_adjutant_rl_dataloader(
    dataset_directory: Path | str,
    *,
    batch_size: int,
    shuffle: bool = False,
) -> DataLoader[AdjutantPpoBatch]:
    samples = list(iter_non_playing_adjutant_rl_samples(dataset_directory))
    dataset = NonPlayingAdjutantRlDataset(samples)
    return cast(
        DataLoader[AdjutantPpoBatch],
        DataLoader(
            dataset,
            ...
            shuffle=shuffle,
            collate_fn=_collate_adjutant_ppo_batch,
            num_workers=0,
        ),
    )
...
def train_adjutant_ppo(
    *,
    ...
    for _epoch in range(settings.epochs):
        ...
        for batch in dataloader:
            ...
            loss = adjutant_ppo_loss(
                ...
                legal_adjutant_mask=batch.legal_adjutant_mask,
    ...
    if totals.sample_count == 0:
        raise AdjutantPpoCompatibilityError("training produced no PPO batches.")

    output = Path(output_checkpoint_path)
    output.parent.mkdir(parents=True, exist_ok=True)
    save_adjutant_ppo_checkpoint(
        output,
        model=model,
        settings=settings,
        manifest=manifest,
...
def initialize_model_from_checkpoint(
    model: AdjutantActorCriticModel,
    checkpoint_path: Path | str,
) -> str:
    checkpoint_file = Path(checkpoint_path)
    raw = _load_raw_checkpoint(checkpoint_file)
    _validate_adjutant_ppo_checkpoint(raw)
    model_config_raw = raw.get("model_config")
    if not isinstance(model_config_raw, dict):
        raise AdjutantPpoCompatibilityError("parent checkpoint model_config must be a dictionary.")
    parent_config = AdjutantMlpConfig.from_dict(model_config_raw)
    ...
    if not isinstance(state, dict):
        raise AdjutantPpoCompatibilityError("parent checkpoint model_state must be a dictionary.")
    try:
...
def save_adjutant_ppo_checkpoint(
    path: Path | str,
    *,
    model: AdjutantActorCriticModel,
    settings: AdjutantPpoTrainSettings,
    manifest: NonPlayingAdjutantRlManifest,
...
def load_adjutant_ppo_checkpoint(
    path: Path | str,
) -> tuple[AdjutantActorCriticModel, dict[str, object]]:
    raw = _load_raw_checkpoint(Path(path))
    _validate_adjutant_ppo_checkpoint(raw)
    config_raw = raw["model_config"]
    if not isinstance(config_raw, dict):
        raise AdjutantPpoCompatibilityError("checkpoint model_config must be a dictionary.")
    model = AdjutantActorCriticModel(AdjutantMlpConfig.from_dict(config_raw))
    state = raw.get("model_state")
    if not isinstance(state, dict):
        raise AdjutantPpoCompatibilityError("checkpoint model_state must be a dictionary.")
    try:
        model.load_state_dict(state)
    except RuntimeError as error:
        raise AdjutantPpoCompatibilityError(
...
def load_adjutant_logits_checkpoint(
    path: Path | str,
) -> tuple[nn.Module, dict[str, object]]:
    raw = _load_raw_checkpoint(Path(path))
    architecture = raw.get("model_architecture")
    if architecture == ADJUTANT_ACTOR_CRITIC_MODEL_ARCHITECTURE:
        return load_adjutant_ppo_checkpoint(path)

    config_raw = raw.get("model_config")
    if not isinstance(config_raw, dict):
        raise AdjutantPpoCompatibilityError("checkpoint model_config must be a dictionary.")
    model = AdjutantMlpModel(AdjutantMlpConfig.from_dict(config_raw))
    state = raw.get("model_state")
    if not isinstance(state, dict):
        raise AdjutantPpoCompatibilityError("checkpoint model_state must be a dictionary.")
    try:
...
class _LossTotals:
    ...
    def update(self, *, batch: AdjutantPpoBatch, loss: AdjutantPpoLoss) -> None:
...
def _parse_rl_sample(raw: object, *, context: str) -> NonPlayingAdjutantRlSample:
    obj = _require_dict(raw, context)
    if obj.get("sampleType") != NON_PLAYING_RL_SAMPLE_TYPE:
        raise AdjutantPpoCompatibilityError(f"{context}: sampleType mismatch.")
    model_input = _float_array(
        obj.get("modelInput"),
        length=ADJUTANT_MODEL_INPUT_FEATURE_COUNT,
        path=f"{context}.modelInput",
    )
    legal_mask = _int_mask_array(
        obj.get("legalAdjutantMask"), length=CARD_COUNT, path=f"{context}.legalAdjutantMask"
    )
    selected = _require_int(obj.get("selectedActionIndex"), f"{context}.selectedActionIndex")
    if selected < 0 or selected >= CARD_COUNT:
        raise AdjutantPpoCompatibilityError(f"{context}: selectedActionIndex is out of range.")
    if int(legal_mask[selected]) != 1:
        raise AdjutantPpoCompatibilityError(f"{context}: selectedActionIndex is illegal.")
    ...
    return NonPlayingAdjutantRlSample(
...
def _collate_adjutant_ppo_batch(samples: list[NonPlayingAdjutantRlSample]) -> AdjutantPpoBatch:
    return AdjutantPpoBatch(
        model_input=torch.from_numpy(np.stack([sample.model_input for sample in samples])),
        legal_adjutant_mask=torch.from_numpy(
            np.stack([sample.legal_adjutant_mask for sample in samples])
        ).to(
            dtype=torch.bool,
        ),
...
def _validate_adjutant_ppo_checkpoint(raw: dict[str, object]) -> None:
    expected = {
        "checkpoint_schema_version": ADJUTANT_PPO_CHECKPOINT_SCHEMA_VERSION,
        "model_architecture": ADJUTANT_ACTOR_CRITIC_MODEL_ARCHITECTURE,
        "algorithm": ADJUTANT_PPO_ALGORITHM,
...
def _require_terminal_reward_transform(value: object, path: str) -> dict[str, object]:
    ...
    if transform != expected:
        raise AdjutantPpoCompatibilityError(f"{path} metadata mismatch.")
    return transform
...
def _load_raw_checkpoint(path: Path) -> dict[str, object]:
    try:
        ...
        raise AdjutantPpoCompatibilityError(f"checkpoint cannot be read: {error}") from error
    except RuntimeError as error:
        raise AdjutantPpoCompatibilityError(f"checkpoint cannot be loaded: {error}") from error
    except pickle.UnpicklingError as error:
        raise AdjutantPpoCompatibilityError(f"checkpoint cannot be loaded: {error}") from error
    if not isinstance(raw, dict):
        raise AdjutantPpoCompatibilityError("checkpoint must be a dictionary.")
    return cast(dict[str, object], raw)


def _require_dict(value: object, path: str) -> dict[str, object]:
    if not isinstance(value, dict):
        raise AdjutantPpoCompatibilityError(f"{path} must be an object.")
    return cast(dict[str, object], value)


def _require_int(value: object, path: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int):
        raise AdjutantPpoCompatibilityError(f"{path} must be an integer.")
    return value
...
def _require_positive_int(value: object, path: str) -> int:
    ...
    if item <= 0:
        raise AdjutantPpoCompatibilityError(f"{path} must be positive.")
    return item
...
def _require_non_negative_int(value: object, path: str) -> int:
    item = _require_int(value, path)
    if item < 0:
        raise AdjutantPpoCompatibilityError(f"{path} must be non-negative.")
    return item


def _require_finite_number(value: object, path: str) -> float:
    if isinstance(value, bool) or not isinstance(value, int | float):
        raise AdjutantPpoCompatibilityError(f"{path} must be a number.")
    item = float(value)
    if not math.isfinite(item):
        raise AdjutantPpoCompatibilityError(f"{path} must be finite.")
    return item


def _float_array(value: object, *, length: int, path: str) -> np.ndarray:
    if not isinstance(value, list) or len(value) != length:
        raise AdjutantPpoCompatibilityError(f"{path} must be a list of length {length}.")
    array = np.asarray(
...
def _int_mask_array(value: object, *, length: int, path: str) -> np.ndarray:
    ...
    for index, item in enumerate(value):
        if item not in (0, 1):
            raise AdjutantPpoCompatibilityError(f"{path}[{index}] must be 0 or 1.")
        items.append(item)
    if sum(items) == 0:
        raise AdjutantPpoCompatibilityError(f"{path} must contain at least one legal action.")
