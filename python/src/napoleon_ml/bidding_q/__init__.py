"""Monte Carlo Q regression utilities for Napoleon bidding."""

from .dataset import (
    BIDDING_Q_ACTION_MAPPING_ID,
    BIDDING_Q_DATASET_SAMPLE_TYPE,
    BIDDING_Q_DATASET_SCHEMA_VERSION,
    BIDDING_Q_REWARD_ID,
    BiddingQAggregatedExample,
    BiddingQDataset,
    BiddingQDatasetManifest,
    BiddingQRawSample,
    BiddingQSplit,
    aggregate_bidding_q_samples,
    create_bidding_q_split,
    load_bidding_q_dataset,
)
from .fixed_hand_margin_training import (
    FIXED_HAND_MARGIN_MODEL_TYPE,
    FixedHandMarginCheckpointError,
    FixedHandMarginDataset,
    FixedHandMarginDatasetError,
    FixedHandMarginSample,
    FixedHandMarginSplit,
    FixedHandMarginTrainConfig,
    FixedHandMarginTrainResult,
    create_fixed_hand_margin_split,
    evaluate_fixed_hand_margin_model,
    fixed_hand_margin_evaluation_report,
    fixed_hand_margin_loss,
    load_fixed_hand_margin_checkpoint,
    load_fixed_hand_margin_dataset,
    predict_fixed_hand_margin_samples,
    same_hand_ranking,
    save_fixed_hand_margin_artifact,
    selected_prediction_arrays,
    train_fixed_hand_margin_model,
)
from .margin_model import (
    BIDDING_MARGIN_HETEROSCEDASTIC_ARCHITECTURE_ID,
    BiddingMarginHeteroscedasticModel,
    BiddingMarginHeteroscedasticModelConfig,
    create_seeded_bidding_margin_model,
    margin_sigma_from_log_variance,
)
from .margin_training import (
    BIDDING_MARGIN_HETEROSCEDASTIC_MODEL_TYPE,
    BiddingMarginTrainConfig,
    BiddingMarginTrainResult,
    export_bidding_margin_onnx,
    gaussian_margin_nll,
    gaussian_success_metrics,
    gaussian_success_probability,
    interval_coverage,
    load_bidding_margin_checkpoint,
    margin_evaluation_report,
    risk_aware_ranking,
    save_bidding_margin_artifact,
    sigma_diagnostics,
    train_bidding_margin_model,
)
from .model import (
    BIDDING_Q_MLP_ARCHITECTURE_ID,
    BiddingQModel,
    BiddingQModelConfig,
    create_seeded_bidding_q_model,
)
from .multi_head_model import (
    BIDDING_MULTI_HEAD_Q_ARCHITECTURE_ID,
    BiddingMultiHeadQModel,
    BiddingMultiHeadQModelConfig,
    BiddingMultiHeadQOutput,
    create_seeded_bidding_multi_head_q_model,
)
from .multi_head_training import (
    BIDDING_MULTI_HEAD_Q_MODEL_TYPE,
    BiddingMultiHeadQSplit,
    BiddingMultiHeadQTrainConfig,
    BiddingMultiHeadQTrainResult,
    Standardization,
    baseline_metrics,
    bidding_multi_head_q_losses,
    create_multi_head_split,
    evaluate_bidding_multi_head_q_model,
    load_bidding_multi_head_q_checkpoint,
    load_validation_state_keys,
    offline_score_metrics,
    save_bidding_multi_head_q_artifact,
    state_key_hash,
    teacher_signal_analysis,
    train_bidding_multi_head_q_model,
)
from .pass_role_binary_model import (
    BIDDING_PASS_ROLE_BINARY_ARCHITECTURE_ID,
    BiddingPassRoleBinaryModel,
    BiddingPassRoleBinaryModelConfig,
    create_seeded_bidding_pass_role_binary_model,
)
from .pass_role_binary_training import (
    BIDDING_PASS_ROLE_BINARY_MODEL_TYPE,
    BiddingPassRoleBinaryTrainConfig,
    BiddingPassRoleBinaryTrainResult,
    audit_pass_binary_dataset,
    create_pass_role_binary_split,
    evaluate_pass_role_binary_model,
    load_bidding_pass_role_binary_checkpoint,
    pass_binary_label,
    pass_binary_metrics,
    predict_pass_adjutant_probability,
    save_bidding_pass_role_binary_artifact,
    train_bidding_pass_role_binary_model,
)
from .pass_outcome_training import (
    PASS_OUTCOME_Q_MODEL_TYPE,
    EmpiricalQTrainConfig,
    EmpiricalQTrainResult,
    PassOutcomeCheckpointError,
    PassOutcomeDataset,
    PassOutcomeDatasetError,
    PassOutcomeSample,
    PassOutcomeSplit,
    RoleMarginTeacher,
    create_pass_outcome_split,
    evaluate_empirical_q_model,
    evaluate_pass_ev_variants,
    load_empirical_q_checkpoint,
    load_pass_outcome_dataset,
    pass_role_margin_dataset,
    predict_empirical_q,
    save_empirical_q_artifact,
    train_empirical_q_model,
    train_pass_role_margin_model,
)
from .pass_simple_score import (
    PassSimplePredictions,
    PassSimpleScoreMatrices,
    compose_pass_simple_score_matrices,
    evaluate_pass_simple_score_offline,
    fit_pass_value_calibrations,
)
from .reward_expectation_score import (
    RoleWinPredictions,
    compose_reward_score_matrices,
    evaluate_reward_expectation_offline,
    evaluate_reward_policy_baselines,
    evaluate_reward_score_matrices,
    napoleon_positive_ev_threshold,
    realized_role_reward,
    reward_d_for_action,
    role_expected_reward,
    role_win_probability_metrics,
)
from .role_margin_model import (
    BIDDING_ADJUTANT_MARGIN_HETEROSCEDASTIC_ARCHITECTURE_ID,
    BIDDING_CITIZEN_MARGIN_HETEROSCEDASTIC_ARCHITECTURE_ID,
    BiddingRoleMarginHeteroscedasticModel,
    BiddingRoleMarginHeteroscedasticModelConfig,
    create_seeded_bidding_role_margin_model,
    role_margin_architecture_id,
)
from .role_margin_training import (
    BIDDING_ROLE_MARGIN_HETEROSCEDASTIC_MODEL_TYPE,
    BiddingRoleMarginTrainConfig,
    BiddingRoleMarginTrainResult,
    evaluate_bidding_role_margin_model,
    load_bidding_role_margin_checkpoint,
    predict_role_margin_samples,
    role_margin_coverage,
    role_margin_teacher,
    save_bidding_role_margin_artifact,
    train_bidding_role_margin_model,
)
from .role_model import (
    BIDDING_ROLE_CLASSES,
    BIDDING_ROLE_COUNT,
    BIDDING_ROLE_Q_ARCHITECTURE_ID,
    BiddingRoleQModel,
    BiddingRoleQModelConfig,
    compose_role_q,
    create_seeded_bidding_role_q_model,
)
from .role_score_integration import (
    AffineCalibration,
    RoleScorePredictions,
    ScoreMatrices,
    StateKeySplit,
    compose_score_matrices,
    create_role_score_state_split,
    evaluate_role_score_offline,
    evaluate_score_matrices,
    fit_affine_calibration,
    fit_role_calibrations,
    role_probability_metrics,
    teacher_utility,
    validate_role_probabilities,
)
from .role_training import (
    BIDDING_ROLE_Q_MODEL_TYPE,
    BiddingRoleQTrainConfig,
    BiddingRoleQTrainResult,
    bidding_role_q_losses,
    canonical_bidding_role,
    export_bidding_role_q_onnx,
    load_bidding_role_q_checkpoint,
    role_dataset_diagnostics,
    role_index_for_terminal_role,
    save_bidding_role_q_artifact,
    train_bidding_role_q_model,
)
from .role_value_model import (
    BIDDING_ADJUTANT_VALUE_ARCHITECTURE_ID,
    BIDDING_CITIZEN_VALUE_ARCHITECTURE_ID,
    BiddingRoleValueModel,
    BiddingRoleValueModelConfig,
    architecture_id_for_role,
    create_seeded_bidding_role_value_model,
)
from .role_value_training import (
    BIDDING_ROLE_VALUE_MODEL_TYPE,
    BiddingRoleValueTrainConfig,
    BiddingRoleValueTrainResult,
    create_role_value_split,
    default_teacher_for_role,
    load_bidding_role_value_checkpoint,
    role_value_coverage,
    role_value_learning_assessment,
    role_value_loss,
    role_value_ranking,
    role_value_teacher,
    save_bidding_role_value_artifact,
    train_bidding_role_value_model,
)
from .training import (
    BiddingQTrainConfig,
    BiddingQTrainResult,
    bidding_q_selected_action_loss,
    coverage_diagnostics,
    evaluate_bidding_q_model,
    export_bidding_q_onnx,
    load_bidding_q_checkpoint,
    ranking_metrics,
    save_bidding_q_artifact,
    train_bidding_q_model,
)

__all__ = [
    "BIDDING_Q_ACTION_MAPPING_ID",
    "BIDDING_Q_DATASET_SAMPLE_TYPE",
    "BIDDING_Q_DATASET_SCHEMA_VERSION",
    "BIDDING_MARGIN_HETEROSCEDASTIC_ARCHITECTURE_ID",
    "BIDDING_MARGIN_HETEROSCEDASTIC_MODEL_TYPE",
    "BIDDING_MULTI_HEAD_Q_ARCHITECTURE_ID",
    "BIDDING_MULTI_HEAD_Q_MODEL_TYPE",
    "BIDDING_PASS_ROLE_BINARY_ARCHITECTURE_ID",
    "BIDDING_PASS_ROLE_BINARY_MODEL_TYPE",
    "PASS_OUTCOME_Q_MODEL_TYPE",
    "BIDDING_ADJUTANT_MARGIN_HETEROSCEDASTIC_ARCHITECTURE_ID",
    "BIDDING_CITIZEN_MARGIN_HETEROSCEDASTIC_ARCHITECTURE_ID",
    "BIDDING_Q_MLP_ARCHITECTURE_ID",
    "BIDDING_Q_REWARD_ID",
    "BIDDING_ROLE_CLASSES",
    "BIDDING_ROLE_COUNT",
    "BIDDING_ADJUTANT_VALUE_ARCHITECTURE_ID",
    "BIDDING_CITIZEN_VALUE_ARCHITECTURE_ID",
    "BIDDING_ROLE_Q_ARCHITECTURE_ID",
    "BIDDING_ROLE_Q_MODEL_TYPE",
    "BIDDING_ROLE_VALUE_MODEL_TYPE",
    "BIDDING_ROLE_MARGIN_HETEROSCEDASTIC_MODEL_TYPE",
    "BiddingQAggregatedExample",
    "BiddingQDataset",
    "BiddingQDatasetManifest",
    "BiddingQModel",
    "BiddingQModelConfig",
    "BiddingQRawSample",
    "BiddingQSplit",
    "BiddingQTrainConfig",
    "BiddingQTrainResult",
    "BiddingMarginHeteroscedasticModel",
    "BiddingMarginHeteroscedasticModelConfig",
    "BiddingMarginTrainConfig",
    "BiddingMarginTrainResult",
    "FIXED_HAND_MARGIN_MODEL_TYPE",
    "FixedHandMarginCheckpointError",
    "FixedHandMarginDataset",
    "FixedHandMarginDatasetError",
    "FixedHandMarginSample",
    "FixedHandMarginSplit",
    "FixedHandMarginTrainConfig",
    "FixedHandMarginTrainResult",
    "BiddingMultiHeadQModel",
    "BiddingMultiHeadQModelConfig",
    "BiddingMultiHeadQOutput",
    "BiddingMultiHeadQSplit",
    "BiddingMultiHeadQTrainConfig",
    "BiddingMultiHeadQTrainResult",
    "BiddingPassRoleBinaryModel",
    "BiddingPassRoleBinaryModelConfig",
    "BiddingPassRoleBinaryTrainConfig",
    "BiddingPassRoleBinaryTrainResult",
    "EmpiricalQTrainConfig",
    "EmpiricalQTrainResult",
    "PassOutcomeCheckpointError",
    "PassOutcomeDataset",
    "PassOutcomeDatasetError",
    "PassOutcomeSample",
    "PassOutcomeSplit",
    "BiddingRoleQModel",
    "BiddingRoleQModelConfig",
    "BiddingRoleQTrainConfig",
    "BiddingRoleQTrainResult",
    "BiddingRoleMarginHeteroscedasticModel",
    "BiddingRoleMarginHeteroscedasticModelConfig",
    "BiddingRoleMarginTrainConfig",
    "BiddingRoleMarginTrainResult",
    "BiddingRoleValueModel",
    "BiddingRoleValueModelConfig",
    "BiddingRoleValueTrainConfig",
    "BiddingRoleValueTrainResult",
    "AffineCalibration",
    "PassSimplePredictions",
    "PassSimpleScoreMatrices",
    "RoleScorePredictions",
    "RoleMarginTeacher",
    "RoleWinPredictions",
    "ScoreMatrices",
    "StateKeySplit",
    "Standardization",
    "aggregate_bidding_q_samples",
    "architecture_id_for_role",
    "audit_pass_binary_dataset",
    "baseline_metrics",
    "bidding_multi_head_q_losses",
    "bidding_role_q_losses",
    "bidding_q_selected_action_loss",
    "canonical_bidding_role",
    "compose_role_q",
    "compose_pass_simple_score_matrices",
    "compose_reward_score_matrices",
    "compose_score_matrices",
    "coverage_diagnostics",
    "create_bidding_q_split",
    "create_multi_head_split",
    "create_pass_role_binary_split",
    "create_pass_outcome_split",
    "create_fixed_hand_margin_split",
    "create_role_score_state_split",
    "create_seeded_bidding_margin_model",
    "create_seeded_bidding_pass_role_binary_model",
    "create_seeded_bidding_q_model",
    "create_seeded_bidding_multi_head_q_model",
    "create_seeded_bidding_role_q_model",
    "create_seeded_bidding_role_margin_model",
    "create_seeded_bidding_role_value_model",
    "create_role_value_split",
    "default_teacher_for_role",
    "evaluate_bidding_q_model",
    "evaluate_bidding_multi_head_q_model",
    "evaluate_pass_role_binary_model",
    "evaluate_empirical_q_model",
    "evaluate_pass_ev_variants",
    "evaluate_pass_simple_score_offline",
    "evaluate_bidding_role_margin_model",
    "evaluate_fixed_hand_margin_model",
    "evaluate_reward_expectation_offline",
    "evaluate_reward_policy_baselines",
    "evaluate_reward_score_matrices",
    "evaluate_role_score_offline",
    "evaluate_score_matrices",
    "export_bidding_margin_onnx",
    "export_bidding_role_q_onnx",
    "export_bidding_q_onnx",
    "gaussian_margin_nll",
    "gaussian_success_metrics",
    "gaussian_success_probability",
    "fixed_hand_margin_evaluation_report",
    "fixed_hand_margin_loss",
    "interval_coverage",
    "load_bidding_role_q_checkpoint",
    "load_bidding_role_margin_checkpoint",
    "load_bidding_role_value_checkpoint",
    "load_bidding_margin_checkpoint",
    "load_bidding_multi_head_q_checkpoint",
    "load_bidding_pass_role_binary_checkpoint",
    "load_empirical_q_checkpoint",
    "load_pass_outcome_dataset",
    "load_bidding_q_checkpoint",
    "load_bidding_q_dataset",
    "load_fixed_hand_margin_checkpoint",
    "load_fixed_hand_margin_dataset",
    "load_validation_state_keys",
    "margin_evaluation_report",
    "margin_sigma_from_log_variance",
    "offline_score_metrics",
    "pass_binary_label",
    "pass_binary_metrics",
    "pass_role_margin_dataset",
    "predict_empirical_q",
    "predict_pass_adjutant_probability",
    "predict_fixed_hand_margin_samples",
    "predict_role_margin_samples",
    "ranking_metrics",
    "fit_affine_calibration",
    "fit_pass_value_calibrations",
    "fit_role_calibrations",
    "role_dataset_diagnostics",
    "role_expected_reward",
    "role_index_for_terminal_role",
    "role_margin_architecture_id",
    "role_margin_coverage",
    "role_margin_teacher",
    "role_probability_metrics",
    "role_win_probability_metrics",
    "role_value_coverage",
    "role_value_learning_assessment",
    "role_value_loss",
    "role_value_ranking",
    "role_value_teacher",
    "teacher_utility",
    "napoleon_positive_ev_threshold",
    "realized_role_reward",
    "reward_d_for_action",
    "risk_aware_ranking",
    "save_bidding_margin_artifact",
    "save_bidding_multi_head_q_artifact",
    "save_bidding_pass_role_binary_artifact",
    "save_empirical_q_artifact",
    "save_bidding_role_q_artifact",
    "save_bidding_role_margin_artifact",
    "save_bidding_role_value_artifact",
    "save_bidding_q_artifact",
    "save_fixed_hand_margin_artifact",
    "same_hand_ranking",
    "selected_prediction_arrays",
    "state_key_hash",
    "sigma_diagnostics",
    "teacher_signal_analysis",
    "train_bidding_margin_model",
    "train_bidding_multi_head_q_model",
    "train_bidding_pass_role_binary_model",
    "train_empirical_q_model",
    "train_pass_role_margin_model",
    "train_bidding_role_q_model",
    "train_bidding_role_margin_model",
    "train_bidding_role_value_model",
    "train_bidding_q_model",
    "train_fixed_hand_margin_model",
    "validate_role_probabilities",
]
