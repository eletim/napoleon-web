"""Multi-head bidding value model for Issue #378."""
...
from .role_model import BIDDING_ROLE_COUNT

BIDDING_MULTI_HEAD_Q_ARCHITECTURE_ID = "bidding-multi-head-q-mlp-v1"


@dataclass(frozen=True)
class BiddingMultiHeadQModelConfig:
    ...
    @classmethod
    def from_dict(cls, value: dict[str, Any]) -> BiddingMultiHeadQModelConfig:
        architecture_id = value.get("architectureId")
        if architecture_id != BIDDING_MULTI_HEAD_Q_ARCHITECTURE_ID:
            raise ValueError(
                f"architectureId must be {BIDDING_MULTI_HEAD_Q_ARCHITECTURE_ID!r}, "
...
class BiddingMultiHeadQModel(nn.Module):
    """Predict role, Napoleon-side cards, contract success, and margin per action."""

    def __init__(self, config: BiddingMultiHeadQModelConfig) -> None:
        super().__init__()
        self.config = config
        ...
        self.role_logits_head = nn.Linear(input_dim, BIDDING_ACTION_COUNT * config.role_count)
        self.napoleon_point_cards_head = nn.Linear(input_dim, BIDDING_ACTION_COUNT)
        self.success_logits_head = nn.Linear(input_dim, BIDDING_ACTION_COUNT)
        self.contract_margin_head = nn.Linear(input_dim, BIDDING_ACTION_COUNT)

    def forward(self, model_input: Tensor) -> BiddingMultiHeadQOutput:
        ...
        return BiddingMultiHeadQOutput(
            role_logits=self.role_logits_head(encoded).reshape(
                batch_size,
                BIDDING_ACTION_COUNT,
                self.config.role_count,
            ),
            napoleon_point_cards=self.napoleon_point_cards_head(encoded),
            success_logits=self.success_logits_head(encoded),
            contract_margin=self.contract_margin_head(encoded),
        )


def create_seeded_bidding_multi_head_q_model(
    config: BiddingMultiHeadQModelConfig,
    *,
    seed: int,
    ...
    return BiddingMultiHeadQModel(config)
