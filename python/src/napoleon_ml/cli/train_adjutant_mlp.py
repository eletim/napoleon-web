"""Train the adjutant-card MLP baseline on CPU."""
...
from napoleon_ml.adjutant.checkpoint import save_adjutant_checkpoint
from napoleon_ml.adjutant.metrics import evaluate_adjutant_model, masked_adjutant_cross_entropy
from napoleon_ml.adjutant.model import AdjutantMlpConfig, create_seeded_adjutant_model
from napoleon_ml.cli._adjutant_common import (
...
def _run(args: argparse.Namespace) -> int:
    ...
    manifest = load_checked_adjutant_manifest(
        args.dataset_directory, command_label="train-adjutant"
    )
    ...
    model = create_seeded_adjutant_model(model_config, seed=settings.seed)
    optimizer = optim.AdamW(model.parameters(), lr=settings.learning_rate)

    train_loader = create_adjutant_dataloader(
    ...
    validation_loader = create_adjutant_dataloader(
        args.dataset_directory,
    ...
    for epoch in range(1, settings.epochs + 1):
        ...
        validation_report = evaluate_adjutant_model(
    ...
    final_report = evaluate_adjutant_model(
        model, validation_loader, split=DatasetSplit.VALIDATION.value
    )
    ...
    if args.json:
        ...
        print_adjutant_report(final_report, as_json=False)
...
def _train_one_epoch(
    ...
    dataloader: Iterable[AdjutantTorchSample],
    ...
    if sample_total == 0:
        raise ValueError("train split contains no adjutant samples.")
