"""Evaluate a saved legal-play policy MLP checkpoint on CPU."""
...
from napoleon_ml.policy.metrics import evaluate_policy_model
...
def _run(args: argparse.Namespace) -> int:
    ...
    manifest = load_checked_manifest(args.dataset_directory, command_label="evaluate-policy")
    model, checkpoint = load_policy_checkpoint(args.checkpoint, manifest=manifest)
    ...
    report = evaluate_policy_model(model, loader, split=args.split.value)
    print_policy_report(report, as_json=args.json)
    return 0
...
def _validate_checkpoint_split_config(
    ...
    if not isinstance(training_config, dict):
        raise PolicyCheckpointCompatibilityError("checkpoint training_config must be a dictionary.")
