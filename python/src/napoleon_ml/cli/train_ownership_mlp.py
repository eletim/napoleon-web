"""Train the hidden-card ownership MLP baseline on CPU."""
...
from napoleon_ml.ownership.checkpoint import save_ownership_checkpoint
from napoleon_ml.ownership.metrics import evaluate_ownership_model, masked_ownership_cross_entropy
from napoleon_ml.ownership.model import OwnershipMlpConfig, create_seeded_ownership_model
...
def _run(args: argparse.Namespace) -> int:
    ...
    model = create_seeded_ownership_model(model_config, seed=settings.seed)
    ...
    for epoch in range(1, settings.epochs + 1):
        ...
        validation_report = evaluate_ownership_model(
    ...
    final_report = evaluate_ownership_model(
        model, validation_loader, split=DatasetSplit.VALIDATION.value
    )
    ...
    if args.json:
        ...
        print_report(final_report, as_json=False)
...
def _train_one_epoch(
    ...
    for batch in dataloader:
        ...
        loss_sum += float(loss.item()) * masked_count
        masked_total += masked_count

    if masked_total == 0:
        raise ValueError("train split contains no masked ownership targets.")

    return loss_sum / masked_total
