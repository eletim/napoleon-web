"""Evaluate a saved legal-bidding policy MLP checkpoint on CPU."""
...
from napoleon_ml.bidding.metrics import evaluate_bidding_model
...
def _run(args: argparse.Namespace) -> int:
    ...
    manifest = load_checked_manifest(args.dataset_directory, command_label="evaluate-bidding")
    model, checkpoint = load_bidding_checkpoint(args.checkpoint, manifest=manifest)
    ...
    report = evaluate_bidding_model(model, loader, split=args.split.value)
    print_bidding_report(report, as_json=args.json)
    return 0
