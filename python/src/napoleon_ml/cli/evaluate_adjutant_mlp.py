"""Evaluate a saved adjutant-card MLP checkpoint on CPU."""
...
from napoleon_ml.adjutant.metrics import evaluate_adjutant_model
...
from napoleon_ml.dataset.pytorch import create_adjutant_dataloader
...
def _run(args: argparse.Namespace) -> int:
    ...
    manifest = load_checked_adjutant_manifest(
        args.dataset_directory, command_label="evaluate-adjutant"
    )
    model, checkpoint = load_adjutant_checkpoint(args.checkpoint, manifest=manifest)
    _validate_checkpoint_split_config(checkpoint, split_config=split_config)
    loader = create_adjutant_dataloader(
    ...
    report = evaluate_adjutant_model(model, loader, split=args.split.value)
    print_adjutant_report(report, as_json=args.json)
    return 0
