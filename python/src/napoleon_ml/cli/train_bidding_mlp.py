"""Train the legal-bidding policy MLP baseline on CPU."""
...
from napoleon_ml.bidding.checkpoint import save_bidding_checkpoint
from napoleon_ml.bidding.metrics import evaluate_bidding_model, masked_bidding_cross_entropy
from napoleon_ml.bidding.model import BiddingMlpConfig, create_seeded_bidding_model
from napoleon_ml.cli._bidding_common import (
...
def _run(args: argparse.Namespace) -> int:
    ...
    train_loader = create_bidding_dataloader(
    ...
    validation_loader = create_bidding_dataloader(
        args.dataset_directory,
    ...
    for epoch in range(1, settings.epochs + 1):
        ...
        validation_report = evaluate_bidding_model(
    ...
    final_report = evaluate_bidding_model(
        model, validation_loader, split=DatasetSplit.VALIDATION.value
    )
    ...
    if args.json:
        ...
        print_bidding_report(final_report, as_json=False)
...
def _train_one_epoch(
    ...
    dataloader: Iterable[BiddingTorchSample],
    ...
    if sample_total == 0:
        raise ValueError("train split contains no bidding samples.")
